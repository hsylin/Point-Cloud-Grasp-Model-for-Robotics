...
import rospy
import RobotControl_func_ros1_chong as RobotControl_func
import shutil
import math
#to do
#顯示的影像還沒校正失真
...
class RGBDThread(QThread):
    ...
    #     min_val = 30
    #     max_val = 255 None, min_val, max_val, cv2.NORM_MINMAX, cv2.CV_8UC1)
    #     return depth_rev
...
class ArmThread(QThread):
    ...
    def goHome(self): #回正
        ...
        #原始座標
        #self.robot.set_TMPos([315.30413818359375, -457.3872680664063, 405.3842163085938, math.radians(-180), math.radians(0), math.radians(45)])
        #chong
        #=====
        self.robot.set_TMPos([546.3766710335036,-100.4645351208886,405.38592529296875,179.99990844726562,-7.330447988351807e-05,45.00006484985351])
    def reGrasp(self):
        # print("restart streaming")
        self._mutex.lock ()
        self._running = True
...
class MainWindow(QMainWindow,Ui_MainWindow):
    ...
    def TestCalibrationF(self): #抓取點位並且復原 ？？
        ...
        # cv2.destroyAllWindows())[0.5940838069345818, -0.1385408801148759, 0.46743569946289065, -3.0309942366798928, 0.028533363832244956, 0.7023456205905642]
        # self.GripperOpenF()
    ...
    def AutoEIHCalibF(self): #修正手眼校正中
        ...
        for Pose in iter(fr):
            ...
            # print('I am in ' + str(self._arm_thread.testArm()))1.0696167303887667
    ...
    def updateRGBDFrame(self, rgbd_image):
        ...
        try:
            #TODO: one point is too small, 7*7 all redhoe
    ...
    def PenTouchTestF(self): #下爪點位測試
        ...
        #chong
        #=====
        #intrinsic = np.array([[909.92,0,640.450],[0,910.077,362.925],[0,0,1]])
        RC2G = np.load(sys.path[0]+'/Camera2Gripper_chong.npy')[0:3,0:3]
        TC2G = np.load(sys.path[0]+'/Camera2Gripper_chong.npy')[:3,3]
        
        d = float(self.depth_value) 
        print(d)
        uv = np.array([realX * d, realY * d, d])
        xyz = np.linalg.inv(intrinsic) @ uv
        XYZ = RC2G @ xyz + TC2G.flatten()
        print(xyz)
        print(XYZ)
        [x,y,z,u,v,w] = self._arm_thread.robot.get_TMPos()
        self._arm_thread.robot.set_TMPos([x+XYZ[1],y+XYZ[0],z - 100,u,v,w])
        #=====
