...
def cal_grasp(msg, cam_pos_):
    ...
    points_voxel_ = get_voxel_fun(points_, 500)
    rospy.loginfo("Convert {} points into {} points".format(len(points_), len(points_voxel_)))
    points_ = points_voxel_
    ...
    #选取桌子以上2cm处的点作为检测点
    select_point_above_table = 0.035
...
if __name__ == '__main__':
    ...
    while not get_transform:
        try:
            ...
            cam_pos, _ = listener.lookupTransform('ar_marker_6', '/camera_color_optical_frame', rospy.Time(0))
            print(cam_pos)
            print(_)
            get_transform = True
    ...
    while not rospy.is_shutdown():
        ...
        if len(real_good_grasp) != 0:
            ...
            print(single_grasp_list_pub.header.frame_id)
